 ...
 */
module.exports.login = async (repoRef, token) => {
	const cmd = `docker login ghcr.io -u ${repoRef} -p ${token}`
	console.log(`Executing: ${cmd}`)
	const { stdout, stderr } = await exec(cmd)
	core.debug(`docker login cmd - stdout: ${stdout}`)
	core.debug(`docker login cmd - stderr: ${stderr}`)
}

/**
 * Build the docker compose images
 */
module.exports.build = async (dockerComposeFile, userBuildArgs = []) => {
	...
	const { stdout } = await exec(imgCmd)
 ...
 */
module.exports.push = async (dockerComposeFile) => {
